...
.Execute in mongo shell
----
db.createUser({ user: 'vuzollDatasetsService', pwd: '?PASSWORD', roles: [ { role: "readWriteAnyDatabase", db: "admin" } ] });
----
...
[source,shell]
----
docker run -it --rm --link vuzoll_mongo:mongo mongo:3.4.1 mongo -u vuzollDatasetsService -p ?PASSWORD --authenticationDatabase admin vuzoll_mongo/vuzoll
----
